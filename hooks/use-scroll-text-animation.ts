...
export function useScrollTextAnimation(options: UseScrollTextAnimationOptions = {}) {
  const { startOffset = 0.8, endOffset = 0.2, transitionDuration = 75, minThreshold = 0, maxThreshold = 1 } = options
    ...
    const handleScroll = () => {
      if (!ticking && textRef.current) {
        rafId = requestAnimationFrame(() => {
          ...
          const documentScrollProgress = currentScrollY / (documentHeight - windowHeight)

          // Apply min/max thresholds to the scroll progress
          let thresholdProgress = 0
          if (documentScrollProgress >= minThreshold && documentScrollProgress <= maxThreshold) {
            // Map the scroll progress within the threshold range to 0-1
            thresholdProgress = (documentScrollProgress - minThreshold) / (maxThreshold - minThreshold)
          ...
          if (elementTop <= startPosition && elementTop >= endPosition) {
            elementProgress = (startPosition - elementTop) / (startPosition - endPosition)
  ...
  const getClipPath = () => {
    ...
    const smoothProgress = Math.max(0, Math.min(100, currentLineProgress * 100))
